...
async def make_contact_batch(start_id: int = 0, list_size: int = 10) -> list[Contact]:
    """
    Make a list of n contacts starting with the given id.
    """
    the_contacts = await asyncio.gather(
        *[make_contact(start_id + i + 1) for i in range(list_size)]
    )
    return the_contacts
...
async def generate_contacts(batch_size: int, total_contacts: int):
    """
    ...
    return pd.json_normalize(json.loads(json_contacts))


async def main():
    start = perf_counter()
    df = await generate_contacts(1000, 20_000)
